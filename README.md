...
This library provides convenient access to the Demo API (Scalar Galaxy) from Python.
...
Every client has an `Async` counterpart (`AsyncDemoAPIScalarGalaxy`) exposing the same resource tree with `await`.
...
async def main() -> None:
    client = AsyncDemoAPIScalarGalaxy()
    planet = await client.planets.list(
        limit=10,
        offset=0,
    )
...
| `o_auth2` | `string \| provider` | - | OAuth 2.0 authentication Defaults to O_AUTH2. |
| `open_id_connect` | `string \| provider` | - | OpenID Connect Authentication Defaults to OPEN_ID_CONNECT. |
...
| `o_auth2` | `str \| None` | `os.environ.get("O_AUTH2")` | OAuth 2.0 authentication |
| `open_id_connect` | `str \| None` | `os.environ.get("OPEN_ID_CONNECT")` | OpenID Connect Authentication |
